...
/// user_info
pub fn info(item: web::Json<ApiRequest>) -> impl Future<Item=HttpResponse, Error=Error> {
    if item.token.is_empty() {
        return ok(HttpResponse::Ok().json(ApiResponse { message: "token is empty".to_owned(), data: "", ..Default::default() }));
    }
